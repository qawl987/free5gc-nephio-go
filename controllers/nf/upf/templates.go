...
{{- range $netInstance := .N6cfg }}
  {{- range $dnn := $netInstance.DataNetworks }}
    {{- range $pool := $dnn.Pool }}
iptables -t nat -A POSTROUTING -s {{ $pool.Prefix }} -o {{index $netInstance.Interfaces 0}} -j MASQUERADE  # route traffic coming from the UE SUBNET to the interface N6
    {{- end }}
  {{- end }}
{{- end }}
...
{{- range $netInstance := .N6cfg }}
  {{- range $dnn := $netInstance.DataNetworks }}
    {{- range $pool := $dnn.Pool }}
ip rule add from {{ $pool.Prefix }} table n6if   # use the created ip table to route the traffic coming from the UE SUBNET
    {{- end }}
